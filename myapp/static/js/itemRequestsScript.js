document.addEventListener("DOMContentLoaded", function () {
  ...
  // //make function for setting the equipment details
  ...
  window.showReqeustDetails = function (itemRequestId) {
    window.makeRequest({
      ...
      onSuccess: (data) => {
        if (data.status) {
          ...
          document.querySelector("#item_request_id_d").dataset.id =
            itemRequestId;
        } else {
  ...
  window.editRequest = function () {
    ...
    // Show the Save button and the image file input
    document.querySelector("#item_request_image_upload").style.display =
      "block";
  ...
  window.saveRequest = function () {
    ...
    const equipmentImageFile = document.querySelector(
      "#item_request_image_file"
    ).files[0];
  ...
  const columns = [
    ...
    { select: 4, title: "date of Request" },
    {
      ...
      render: function (data, cell, _dataIndex, _cellIndex) {
        ...
        return `<a href="#" data-bs-toggle="modal" data-bs-target="#ItemRequestsModal" onclick="showReqeustDetails(${itemRequestId})">View Details</a>`;
      },
    },
    {
      select: 6,
